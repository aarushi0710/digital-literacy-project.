# Digital Literacy Project


**Name:** Aarushi Srivastava
**Registration No:** 25BAI10536
**Degree:** B.Tech CSE (AIML)

This repository contains all the tasks completed for the Digital Literacy course.


Core Objectives:

*Professional Identity: Building a verified presence on GitHub and LinkedIn to showcase 
technical projects.

*Digital Communication: Mastering the art of professional storytelling through presentations
and formal email etiquette.

*Security & Ethics: Understanding the landscape of cybercrime and implementing best 
practices for data privacy.

*Documentation: Learning to use Markdown and Version Control (Git) to organize and present
academic work effectively.

Each folder in this repository contains a specific "Task" that represents a milestone in 
developing these skills.




## Project Tasks


## Task 1:Presentation
* [Task 01: Presentation](./Task%20-%201%20-%20presentation)
* Summary:Focused on effective visual communication and storytelling. This task involved
   creating a structured presentation to convey complex ideas clearly using digital tools.


## Task 2:Portfolio
* [Task 02: Portfolio](./Task%20-%202%20-%20Portfolio)
*Summary: Developed a professional digital presence. This module covered the creation of a
personal portfolio to showcase academic projects, skills, and career objectives.



## Task 3: Coding & Collaboration Platforms
* [Link to the folder](./Task-3-platforms)
* **Google Form Quiz:** [Click here to view my Digital Literacy Quiz](https://docs.google.com/forms/d/e/1FAIpQLSdAH-X2vRbZk2fUxfu0ZkOeEMG6iCqz60Hb6fBRxj3fLmylRQ/viewform?usp=header)
* **HackerRank Profile:** Successfully completed the Python 'Hello World' challenge.
*Summary:Exploration of various digital platforms and tools. This task involved setting up
 and optimizing profiles on professional networking and version control sites like LinkedIn
 and GitHub.

## Task 4: Email Etiquette
* [Task 04: Email Etiquette](./Task-4-email-etiquette)
*Summary:Mastered professional communication standards. This module focused on drafting
clear, respectful, and effective emails for academic and corporate environments


### Task 5: Cybercrime Awareness
* Case Study:[UPI / Online Payment Fraud Analysis](./Task-5-cybercrime/casestudy.md)
* Prevention Guide: [Stay Safe Online Checklist](./Task-5-cybercrime/prevention-checklist.md)
*Summary:Awareness and prevention of digital threats. This task involved researching common
 cybercrimes and documenting best practices for maintaining data privacy and online
security.




Submitted for the Digital Literacy Coursework - March 2026
